...
VERSION   ?= $(call get_svc_version,$(JVVMAJOR))
...
# ARM v7/v6 packages unavailable since 11
ifeq ($(JVVMAJOR), 11)
	SKIP_armv7l := 1
	SKIP_armhf  := 1
...
JAVA_VERSION_CMD := docker run --rm --pull=never --entrypoint java $(IMAGETAG) -version 2>&1
JAVA_VERSION_CMD_MAP := \
	'7') $(JAVA_VERSION_CMD) | grep 'java version '    | cut -d\" -f 2 ;; \
	  *) $(JAVA_VERSION_CMD) | grep 'openjdk version ' | cut -d\" -f 2 ;; \
	#
# $1 = JVVMAJOR
define get_svc_version
$(shell case "$(1)" in $(JAVA_VERSION_CMD_MAP) esac)
endef
# -- }}}
